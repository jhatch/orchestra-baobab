...
Toggle.prototype.render = function () {
  this.$el.html(Toggle.view({
    name:      _.escape(this.name),
    whenTrue:  _.escape(this.whenTrue),
    whenFalse: _.escape(this.whenFalse)
  }));

  this.$el.find('input').removeAttr('checked');
  this.$el.find('input[value="' +  _.escape(this.value) + '"]').attr('checked', true);
...
Toggle.prototype.click = function (cb) {
  var self = this;
  this.$el.on('click', 'input[name=' + _.escape(this.name) + ']', function () {
