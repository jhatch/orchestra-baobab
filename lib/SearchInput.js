...
SearchInput.prototype.beindChangeHandler = function () {
  var self = this;
  this.$el.on('keypress', '#ctrl_' + this.id, function (evt) {
    setTimeout(function () {
      self.val = self.$el.find('#ctrl_' + self.id).val();
    });
  });
};
...
// XXX we need to setup a delegated even for onchange, handle throttling
// and update our own internal state with the current value of the input
